...
export function NowSection({
  ...
  return (
    <section
      ...
      aria-label="Now"
      className={cn(
        "border-b border-border bg-card px-4 py-3 text-card-foreground",
        className,
      )}
    >
      <div className="flex items-end justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs font-semibold tracking-[0.14em] text-primary-emphasis uppercase">
            Now
          </p>
          <p className="text-sm font-semibold tracking-tight">
            {count === 0 ? "Clear" : countLabel}
          </p>
        </div>
      </div>
        ...
        const body = (
          <>
            <div className="flex items-center gap-2">
              ...
              <Badge tone={item.priority === "now" ? "accent" : "neutral"}>
                {item.classification}
              </Badge>
