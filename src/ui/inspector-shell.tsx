...
export function InspectorPanelTitle({
  ...
  return (
    <div
      className={cn(
        "mb-2 mt-4 text-[10.5px] font-bold uppercase tracking-[0.05em] text-muted-foreground first:mt-0",
        className,
      )}
    >
      {children}
  ...
  if (empty !== undefined && header === undefined) {
    return (
      <aside
        ...
        className={cn(
          "flex min-h-0 flex-col overflow-auto border-l border-border bg-card",
          className,
        )}
      >
        <div className="flex flex-1 flex-col items-start justify-center px-5 py-10">
          {empty}
        </div>
      </aside>
    );
