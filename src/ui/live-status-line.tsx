...
export function LiveStatusLine({ label, className }: LiveStatusLineProps) {
  return (
    <div
      ...
      aria-live="polite"
      className={cn(
        "flex items-center gap-2 border-b border-border bg-card px-6 py-2.5 text-sm text-muted-foreground",
        className,
      )}
    >
      <span
  ...
  return (
    <div
      className={cn(
        ...
        label === null ? "grid-rows-[0fr]" : "grid-rows-[1fr]",
        className,
      )}
    >
      <div className="overflow-hidden">
        {label === null ? null : <LiveStatusLine label={label} />}
      </div>
    </div>
  );
}
